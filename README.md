...
| `grounded_verify` | Verify a claim against verbatim source the caller names (file paths, line ranges, or glob patterns within a configured root): the server reads the exact text, so the caller can't paraphrase the evidence. Returns the verdict (supported/refuted, or `inconclusive` routing the caller to `check` when a computable property — a count or measure — is the decisive fact), findings, an audit manifest of what was read, and a completeness signal naming omitted evidence. | `GROUNDED_VERIFY_ROOT` |
