...
- [X] T034 Review against the design corpus with the `design-reviewer` agent and against Rust conventions with `code-reviewer` — required before merge for changes touching the tool surface and schemas (Constitution, Development Workflow)
- [X] T035 Live-verify after rebuild and restart using the question in [quickstart.md](quickstart.md), confirming non-zero confidence, published coverage, and statuses that reconcile — where the same question previously reported confidence 0. **Verified 2026-07-25**: the run scoped 7 sub-questions and the synthesis returned 7 gaps, so the old formula would have reported exactly 0 again; the new one reports `confidence 0.7750`, `coverage 0.142857` (1 of 7 statuses settled, reconciling to six digits), and `refutation_rate 0.0597` (4 dropped / 67 verified). The first rebuild did not take — the release exe was locked by the running server, so the binary was checked for the new symbols before spending a second run.
