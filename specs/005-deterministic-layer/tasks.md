...
- [ ] T011 [P] Acceptance example examples/acceptance_check.rs (live: ANTHROPIC_API_KEY only): ≥20 ground-truth claims spanning both engines (SC-001 100% verdict accuracy), ≥6 clearly uncheckable claims (SC-002 100% declined), auditability sweep (SC-003), one repeated check asserting identical engine result (SC-007); record results in specs/005-deterministic-layer/quickstart.md
