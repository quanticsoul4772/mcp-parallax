...
- [X] T019 **Live dogfood** (after merge + rebuild + restart): follow `quickstart.md` — supersession (update wins, original inspectable), Berlin/Lisbon (both stay), merge (byte-identical canonical), capture (candidate proposed, quarantined, never pushed, forget-cleaned), plus a look at `consolidation_records`. Record the result here; clean all seeds. (Live-model properties: whether real judgments classify real pairs correctly; the mechanics are proven by T004–T014.)
  ...
  **Result (2026-07-23, live on the merged `d79d8f8` release build): PASS — all four sequences.**
  - **Migration**: pre-017 store migrated in place — 13 columns on `memories`, the one pre-existing row intact with `last_reinforced_at` backfilled to `created_at`; `consolidation_records` created.
  - **Supersession**: seeded "cobalt-ferret deploys from the legacy jenkins server", then saved the update ("…now deploys from the new github actions pipeline"). Recall returned only the update; the original went `status=superseded` with `replaced_by` set (inspectable, not deleted); one `supersede` audit row — basis: "The word 'now' signals the deployment method changed from jenkins to github actions."
  - **Context-specific**: "the team's home office is in Lisbon" + "the team is working from Berlin this week" — both stayed `active`, zero consolidation actions. The judgment kept coexisting scopes apart.
  - **Merge**: "always clear the amber cache before starting a cutover" vs "the amber cache must be cleared before any cutover begins" — merged into one `active` canonical, one `merge` audit row with a coherent basis.
  - **Capture**: the turn hop on the new binary evaluated all three judgments (`signals_evaluated: ["self_contradiction","preference_violation","capture_proposal"]`, `review_ran=1`, verdict `silence`, $0.024), proposed exactly one candidate from the turn's diagnosed finding, stored it **quarantined** (`trust=untrusted`) with a `capture_proposed` audit row — then forget-cleaned per quickstart step 4. Decline bias held on the preceding procedural turns (zero proposals).
  - **Cleanup**: all six seeds + the captured candidate deleted; store back to its single pre-existing memory; final `consolidation_records` tally: 1 supersede, 1 merge, 1 capture_proposed.
  - **Findings**: (1) the 006 batch-boundary loop screen flagged the six-call `forget` cleanup as a possible loop — its input normalization collapses argument values, so a finite batch of distinct-id deletions is indistinguishable from a genuine loop; a real alarm-fatigue precision datum for any future screen tuning. (2) No turn-boundary checkpoint record existed post-restart until explicitly probed — the prior turn had ended in context compaction, not a clean stop; the capture judgment was then verified by invoking `checkpoint_turn` directly against the live transcript (the tool's contract: direct invocation behaves identically).
