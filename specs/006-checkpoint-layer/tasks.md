...
- [X] T006 S2 spike (gates D4/SC-003): `examples/spike_embed_latency.rs` — 50 sequential Voyage query embeds, report p50/p95; record the measurement and the resulting decision (τ value; semantic-in-gate vs lexical fallback vs SC-003 amendment) in research.md D4 and data-model.md §1. Requires `VOYAGE_API_KEY`; run once and commit findings.
