...
- [ ] T001 [US1] Add a fixed `LENSES` array (literal, counterexample, definitional, evidential, scope — each a directive paragraph) and a `<<lens>>` placeholder in `PROMPT_TEMPLATE`, in `src/modes/verify.rs`. The lens occupies the critical-instruction slot only; `claim`/`context` stay the only subject inputs (stance-blindness, research D3).
- [ ] T002 [US1] In `verify::run` (`src/modes/verify.rs`), build a per-pass prompt assigning `LENSES[i % LENSES.len()]` to pass *i*; leave `aggregate_core` and the quorum/confidence math unchanged (only the inputs diversify).
- [ ] T003 [P] [US1] Unit tests in `src/modes/verify.rs`: the *k* built prompts are pairwise distinct (lenses injected); `build_prompt` still contains only the lens + claim + context slots (no stance); and the aggregation vote-vector tests (2:1 → ≈0.67, even-`k` 2:2 tie → refuted, sub-quorum → dominant failure) have direct coverage (FR-004 / SC-005). No live model.
- [ ] T004 [US1] Verify the regression set (FR-003) with the mocked model: a clear-error claim still converges to `refuted` with the named finding; a clearly-true claim returns `supported` with no manufactured findings; an authority-vouching context still refutes on the merits — in `src/modes/verify.rs` tests or `tests/integration.rs`.
...
- [ ] T005 [US2] Add `needs_computation: bool` to the `GroundedPass` schema and instruct the prompt to set it when the claim's truth hinges on an exact computation of the source the pass cannot perform by reading (a precise count/measure), in `src/modes/grounded_verify.rs`. Keep the pass schema flat + closed.
- [ ] T006 [US2] Add a server-assembled `GroundedVerdictKind { Supported, Refuted, Inconclusive }` and a `reason` field on `GroundedVerdict`; the per-pass `VerdictKind` (shared with `verify`) stays `{ Supported, Refuted }`, so `verify` is untouched (FR-009) — in `src/modes/grounded_verify.rs`.
- [ ] T007 [US2] In grounded aggregation (`src/modes/grounded_verify.rs`), after the existing pass aggregation: if a majority of completed passes set `needs_computation` → `Inconclusive` (reason: computable property — route to `check`); else if aggregated `missing_evidence` is non-empty (decisive) → `Inconclusive` (reason: decisive evidence missing); else the majority `Supported`/`Refuted` (008 behavior).
- [ ] T008 [P] [US2] Unit tests in `src/modes/grounded_verify.rs`: majority `needs_computation` → `inconclusive` (route reason); non-empty `missing_evidence` → `inconclusive`; no flag + empty missing → `supported`/`refuted` unchanged; the pass schema validates flat + closed with the new boolean.
- [ ] T009 [P] [US2] Integration tests in `tests/integration.rs` (010 block): the `src/server.rs > 1000 lines` reproduction (mock passes return `needs_computation=true`) → the tool returns `inconclusive`, never `refuted` at 1.0 (FR-008 / SC-003); a genuine judgment claim (no flag) returns `supported`/`refuted` unchanged (FR-007).

**Checkpoint**: both stories complete and offline-tested.

## Phase 4: Polish & Cross-Cutting Concerns

- [ ] T010 [P] Docs: note the `inconclusive` verdict in the `grounded_verify` README Tools row and `CLAUDE.md`; note that `verify` runs diverse lenses.
- [ ] T011 [P] Extend `examples/acceptance_grounded_verify.rs` with the `inconclusive` reproduction (server.rs line-count → inconclusive), and add the verify regression assertions.
