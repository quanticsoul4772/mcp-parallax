...
- Q: Which source-locator types should v1 support? → A: File paths and file-with-line-ranges; globs deferred to a follow-up.
- Q: Should the completeness signal (US3 / FR-010) ship in v1? → A: Yes — include it in v1.
- Q: When one of several named locators can't be resolved, what should the call do? → A: Fail the whole call, loudly (all-or-nothing; no verdict over a partial evidence set).
- Q: How many source roots can an operator configure for v1? → A: A single configured root.
...
supplies the claim and a set of source locators — file paths or file/line
ranges — pointing into the configured source root. The server reads exactly that
text, verbatim, assembles it as the verification context, runs the stance-blind
ensemble over it, and returns a verdict (supported or refuted), specific
findings, and an agreement-derived confidence.
...
**Independent Test**: Call with a known set of locators (a full file and a line
range) and confirm the returned manifest names exactly the files and ranges that
were read, with sizes, such that the evidence set can be reconstructed from the
manifest alone.

**Acceptance Scenarios**:

1. **Given** a call with three locators (two full files and a line range), **When** the verdict is returned, **Then** the manifest lists all three resolved sources with their byte sizes and the line range actually read.
2. **Given** a locator that is a file-with-line-range, **When** the manifest is returned, **Then** it records the exact line span read, not just the file name.
...
caller-omits-evidence gap; it is the lowest-priority slice (built last), but it
is in v1 scope. The tool's hard guarantee is fidelity of *what it read*;
...
- **Any named source fails to resolve → the whole call fails.** A locator that is missing, empty, out of range, or non-text aborts the entire call with a loud error naming the offending locator. The call is all-or-nothing: no verdict is ever rendered over a partially-resolved evidence set, because a verdict over silently-reduced evidence is exactly the "looks clean but isn't" failure this feature exists to prevent.
- **Named source does not exist**: a locator pointing to a missing file is a loud error naming it.
- **Named source is empty**: a zero-byte file is a loud error — verifying against no evidence is meaningless and must not look like a clean result.
...
- **FR-001**: The tool MUST be gated and off by default — present in the catalog only when an operator has configured a single allowed source root, following the existing off-by-default convention for capabilities that read or reach outside the process.
- **FR-002**: The tool MUST accept a claim plus a set of source locators, where a locator is a file path or a file-with-line-range, interpreted relative to the single configured source root. (Glob locators are deferred to a follow-up.)
...
- **FR-004**: Every resolved locator MUST be confined to the single configured source root; any locator resolving outside it (via traversal or symlink) MUST be rejected before any content is read.
...
- **FR-009**: If ANY named source is missing, empty, out of range, or non-text, the tool MUST abort the entire call with a loud error identifying the offending locator and render no verdict — the call is all-or-nothing, never a verification over an absent or partially-resolved evidence set presented as complete.
- **FR-010**: The result MUST include a completeness signal naming additional evidence the pass would have needed but was not given; when the provided evidence is sufficient, the signal is empty.
...
- **Source locator**: a caller-supplied reference to evidence — a file path or a file-with-line-range — interpreted within the single configured source root. (Globs are a deferred follow-up, not part of v1.)
...
- **The sources live on the server's filesystem.** In the MCP model the server reads its own filesystem, not the client's; the single "source root" scopes that filesystem. The calling model supplies locators (relative paths or file/line ranges); the operator configures the one root they resolve within.
...
- **Globs are deferred.** v1 locators are exact paths and line ranges; glob resolution (directory walk, deterministic ordering, zero-match semantics, symlink-expansion guarding) is a named follow-up, not part of this scope.
- **The tool guarantees fidelity of what it reads, not completeness of what was chosen.** Selecting the right sources remains the caller's judgment; FR-010's completeness signal (in v1 scope) makes omissions visible but does not enforce them. This boundary is intentional.
- **Enablement and bounds follow the existing env-configuration convention**: a present-but-malformed configuration value is a startup error, never a silent fallback; the single source root and the byte/locator ceilings are environment-configured and off (or at safe defaults) by default.
