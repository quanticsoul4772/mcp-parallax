...
- [ ] T006 [US1] Tests (mocked model) in `src/modes/decide.rs` or `tests/integration.rs`: a full `decide` run returns `recommended` + `runner_up` + `runner_up_reason` + `deciding_factors` + `methodology` + `assessments` (one per option) and **no** `verdict`/`next_step` field (FR-007); an arity mismatch (scores vs options) is a failed pass; an **out-of-range score** (e.g. `105` or `-5`) is a failed pass (loud, not clamped — analyze M1); `< 2` options is rejected before any model call (FR-008/SC-005).
