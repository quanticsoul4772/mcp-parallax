...
- [ ] T004 Implement the extended-glob → anchored-regex translator in `src/grounded/glob/translate.rs`: `*`, `**` (recursive), `?`, `[class]`/`[!class]`, brace `{a,b}` (nestable), extglob `@(...)` `?(...)` `*(...)` `+(...)` `!(...)`, and leading `!` (whole-pattern negation). A malformed pattern (unbalanced `(`/`[`/`{`, empty alternation) is a named `InvalidInput`. Pure — string in, compiled `Regex` out.
- [ ] T005 [P] Unit tests for the translator in `src/grounded/glob/translate.rs` (ground-truth table): each construct matches the intended paths and rejects others; `**` crosses segments while `*` does not; nested brace; each extglob operator incl. `!(...)` negation; malformed patterns rejected named. No disk.
