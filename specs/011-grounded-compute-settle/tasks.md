...
- [ ] T001 Add the four flat nullable fields to `GroundedPass` in `src/modes/grounded_verify.rs` — `compute_property` (nullable enum `lines|bytes|matches`), `compute_match_literal` (nullable string), `compute_operator` (nullable enum `>|>=|<|<=|==|!=`), `compute_threshold` (nullable integer) — and extend the prompt: when `needs_computation` is set and the claim is a line/byte/literal-match count of the source compared to a numeric bound, fill these to name *what* to count and the comparison (never the value or verdict). Keep the pass schema flat + closed; confirm `assert_flat` accepts the nullable scalars/enums.
...
- [ ] T003 [US1] Add a server-internal `ComputeSpec { property: Property{Lines,Bytes,Matches(String)}, operator: Op, threshold: i64 }` and a pure `agreed_spec(passes) -> Option<ComputeSpec>` in `src/modes/grounded_verify.rs`: returns `Some` only when a majority of the `needs_computation` passes carry an identical, complete, in-class spec (same property, operator, threshold, and literal for `matches`); otherwise `None`.
...
- [ ] T006 [US1] In grounded aggregation (`src/modes/grounded_verify.rs`), when a `needs_computation` majority holds AND `units.len() == 1` AND `agreed_spec` is `Some`: count the property over the single raw unit, build `format!("{value} {op} {threshold}")`, call `deterministic::arithmetic::evaluate`, and return `supported`/`refuted` from `holds` with `executed_form` + `engine_result` and a one-line `findings` note ("counted N lines"), `confidence` 1.0. (The else branch is US2.)
- [ ] T007 [P] [US1] Unit tests in `src/modes/grounded_verify.rs`: agreed `lines > 1000` over a 1224-line raw unit → `supported`, `executed_form` `1224 > 1000`, `engine_result` `true`; `lines > 5000` → `refuted`; a `bytes` and a `matches` spec each settle; the count conventions from T004 hold.
...
- [ ] T008 [US2] Complete the aggregation fallthrough in `src/modes/grounded_verify.rs`: when a `needs_computation` majority holds but `agreed_spec` is `None` (disagreement, out-of-class property, missing field) OR `units.len() != 1` (multi-source) OR `arithmetic::evaluate` errors → return 010's `inconclusive` (route-to-`check` reason); never a verdict over an underived/unsettled value. The non-`needs_computation` judgment path stays exactly 010 (FR-006).
- [ ] T009 [P] [US2] Unit tests in `src/modes/grounded_verify.rs`: passes disagree on the spec → `inconclusive`; an out-of-class property (e.g. `compute_property` null while flagged) → `inconclusive`; `units.len() == 2` with an otherwise-valid spec → `inconclusive`; a non-`needs_computation` majority → `supported`/`refuted` unchanged (no compute attempted, no `executed_form`).
