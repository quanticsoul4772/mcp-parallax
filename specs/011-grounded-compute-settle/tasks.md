...
- [X] T011 [P] Extend `examples/acceptance_grounded_verify.rs` with the compute-settle reproduction (server.rs line-count → `supported` with executed form), and update docs: the `grounded_verify` README Tools row and `CLAUDE.md` note that a computable claim is now *settled* (not just abstained) for the line/byte/match class.
