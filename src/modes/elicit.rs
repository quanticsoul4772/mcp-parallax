...
mod tests {
    ...
    ///
    /// `elicit` shipped without a contract, so for four of fifteen tools the
    ...
    fn derived_schemas_match_the_contract_file() {
        let contract: Value = serde_json::from_str(include_str!(
            "../../specs/014-preference-elicitation/contracts/elicit.tool.json"
        ))
        .unwrap();

        let input = serde_json::to_value(schemars::schema_for!(ElicitParams)).unwrap();
        ...
        assert_eq!(
            ...
            props(&input),
            "elicit input properties drifted from the contract"
        );
        assert_eq!(
            contract["inputSchema"]["required"], input["required"],
            "elicit required inputs drifted"
        );
        crate::schema::assert_constraints_agree(&contract["inputSchema"], &input, "elicit");

        let output = serde_json::to_value(schemars::schema_for!(ElicitResult)).unwrap();
        assert_eq!(
            props(&contract["outputSchema"]),
            props(&output),
            "elicit output properties drifted from the contract"
        );
